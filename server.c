...
// Increase buffer size to handle larger DH key exchange messages and encrypted data
#define PORT 8080
...
void receiveClientMessage(int senderIndex) {
    ...
    char msg[BUFFER_SIZE] = {0};  // Increased buffer size
    ...
    if (err <= 0) {
        ...
        // For normal messages, print to console
        if (strncmp(msg, "DH_PUBKEY:", 10) == 0) {
            printf("Client %d sending DH key exchange data (%d bytes)\n", senderIndex + 1, err);
        } else if (strncmp(msg, "ENC:", 4) == 0) {
            printf("Client %d sending encrypted message (%d bytes)\n", senderIndex + 1, err);
        } else {
        ...
        if (receiverSocket != 0) {
            ...
            send(receiverSocket, msg, err, 0);  // Use err instead of strlen to handle binary data
        } else {
...
int main(void) {
    ...
    // Alternative: use a specific IP address
    // InetPtonA(AF_INET, "127.0.0.1", &serv.sin_addr);
    ...
    while (1) {
        ...
        if (err < 0) {
            ...
            if (FD_ISSET(listener, &fr)) {
                ...
                for (int i = 0; i < 2; i++) {
                    if (client[i] != 0 && FD_ISSET(client[i], &fr)) {
